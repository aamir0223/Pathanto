...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($data['unlike'])) {
    // Update the 'likes' count in the 'questions' table (decrement by 1)
    $sqlUpdateLikes = "UPDATE questions SET likes = GREATEST(likes - 1, 0) WHERE id = ?";
    ...
    if (isset($_POST['unlike'])) {  // Change 'like' to 'unlike' in unlike.php
        $_SESSION['liked_' . $questionId] = false;  // Change 'true' to 'false' in unlike.php
    }
 ...
 // Get the updated 'hasLiked' status
    ...
    echo json_encode(['success' => true, 'message' => 'Unlike processed successfully', 'questionId' => $questionId, 'likesCount' => $likesCount]);
} else {
    // Invalid request
